...
sampler2D   _EyeScleraAlbedo;

sampler2D   _EyeScleraSpecularOcclusion;
...
half3 Albedo(float4 texcoords, bool isSclera)
{
    half3 albedo = _Color.rgb;// * tex2D (_MainTex, texcoords.xy).rgb;
    if(isSclera){
        albedo *= tex2D (_EyeScleraAlbedo, texcoords.xy).rgb;
    } else {
        albedo *= tex2D (_MainTex, texcoords.xy).rgb;
    }
...
half3 NormalInTangentSpace(float4 texcoords, bool isSclera)
{
    half3 normalTangent;
    if(isSclera){
       normalTangent = UnpackScaleNormal(tex2D (_EyeScleraBumpMap, texcoords.xy), _EyeScleraBumpScale);
    } else {
