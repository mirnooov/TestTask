...
/// </summary>
public class ForExampleQuest : MonoBehaviour
{
    ...
    /// </summary>
    public List<Criteria> orikList = new List<Criteria>();
    /// <summary>
    ...
    /// </summary>
   // [HideInInspector]
    public List<ForExampleQuest> unCompletedQuests = new List<ForExampleQuest>();

    /// <summary>
    /// List to put the Quests you want to be able to use as chain quests.
    /// </summary>
  //  [HideInInspector]
    public List<ForExampleQuest> questsToUnlock = new List<ForExampleQuest>();
    ...
    public virtual void StartCriterias()
    {
        ...
        {
            foreach (Criteria criteria in orikList)
            {
                if (criteria.Level == 0)
                {
    ...
    public virtual void OnCompletion(CQPlayerObject player, CQExamplePlayer unit)
    {
        if (playersUnCompleted.Contains(player))
        {
            ...
            foreach (ForExampleQuest quest in questsToUnlock)
            {
                if (quest != null)
                {
            ...
            if (noSpawnIfNoPlayer == true)
            {
                ...
                {
                    List<Criteria> tmpCriteria = new List<Criteria>(orikList);
    ...
    public virtual void CreateCriteria(Criteria critera)
    {
        ...
        tmpC = critera;
        tmpC.ForExampleQuest = GetComponent<ForExampleQuest>();
        orikList.Add(tmpC);
    }
    ...
    /// </summary>
    /// <param name="forExampleQuest">The quest this quest removes from its unCompletedQuests</param>
    /// <param name="player">The player unlocking the quest</param>
    public virtual void UnlockQuest(ForExampleQuest forExampleQuest, CQPlayerObject player)
    {
        unCompletedQuests.Remove(forExampleQuest);
    ...
    public virtual void ResetCriterias(CQPlayerObject player)
    {
        ...
        foreach (Criteria c in orikList)
        {
    ...
    public virtual void AddPlayerToCriterias()
    {
        foreach (CQPlayerObject player in playersUnCompleted)
        {
            ...
            {
                unCompletedCriterias.Add(player, new List<Criteria>(orikList)); // If the player does not exist in the dictonary, he has not done any criterias. Therefor, we add them all here.
            }
