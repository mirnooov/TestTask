...
public class QuestDeleteWindow : EditorWindow
{
    ...
    private ForExampleQuest _forExampleQuestToDelete;
    ...
    private void OnGUI()
    {
        ...
        GUI.skin = null;
        EditorGUILayout.LabelField("Do you wish to delete " + _forExampleQuestToDelete.questName + "?", EditorStyles.wordWrappedLabel);
        GUILayout.Space(10);
        if (GUILayout.Button("No")) this.Close();
        if (GUILayout.Button("Yes") || Event.current.keyCode == (KeyCode.Return))
        {
            questEditor.DeleteQuest(_forExampleQuestToDelete);
    ...
    public void SetQuestEditor(CustomQuestEditor editor, ForExampleQuest q)
    {
        questEditor = editor;
        _forExampleQuestToDelete = q;
    }
}