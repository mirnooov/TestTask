...
public class CustomQuestEditor : EditorWindow
{
    ...
    /// </summary>
    [FormerlySerializedAs("selectedQuest")] public ForExampleQuest selectedForExampleQuest;
    ...
    /// </summary>
    private ForExampleQuest _tmpForExampleQuest;
    ...
    private void Awake()
    {
        ...
        foreach (ForExampleQuest q in FindObjectsOfType<ForExampleQuest>()) //Goes through all quests in scene and checks if they are in the editors nodes, if not, adds them
        {
        ...
        UpdateCriteriaList();
        UpdateRewardList();
        R_questPrefabList = new ReorderableList(allQuests, typeof(ForExampleQuest), true, false, false, false);
        ...
        R_questInSceneList = new ReorderableList(allQuests, typeof(ForExampleQuest), true, false, false, false);
            ...
            if (updateCounter <= 0)
            {
                switch (renameType)
                {
                    case TypeToRename.Quest:
                        RenamePrefab(selectedForExampleQuest.gameObject);
                        break;
                ...
                if (!EditorApplication.isCompiling) //Checks if the editor is not compiling - To avoid errors.
                {
                    ...
                    switch (attachType) //Branches out, depinding on what type its currenly attaching
                    {
                        case TypeToAttach.Quest: //For attaching a quests
                            ...
                            DestroyImmediate(tmpGameObject.GetComponent<ForExampleQuest>());
                            tp = GetComponentTypeByName(tmpGameObject.name.Replace(" ", ""));
                            tmpGameObject.AddComponent(tp);

                            PrefabUtility.CreatePrefab("Assets/CustomQuest/Assets/Prefabs/Quests/" + tmpGameObject.name + "/" + tmpGameObject.name + ".prefab", tmpGameObject);
                            AssetDatabase.MoveAsset("Assets/" + tmpGameObject.name.Replace(" ", "") + ".cs", "Assets/CustomQuest/Assets/Prefabs/Quests/" + tmpGameObject.name + "/" + tmpGameObject.name.Replace(" ", "") + ".cs");
                            GameObject newQuest = AssetDatabase.LoadAssetAtPath("Assets/CustomQuest/Assets/Prefabs/Quests/" + tmpGameObject.name + "/" + tmpGameObject.name + ".prefab", typeof(GameObject)) as GameObject;
                            UpdateQuestList();
                            ForExampleQuest newForExampleQuestScript = newQuest.GetComponent<ForExampleQuest>();
                            newForExampleQuestScript.EditorStart();
                            if (_tmpForExampleQuest.handInObjects != null)
                            {
                                foreach (HandInObject h in _tmpForExampleQuest.handInObjects)
                                {
                                    HandInObject tmpH = Instantiate(h, newForExampleQuestScript.gameObject.transform);
                                    newForExampleQuestScript.handInObjects.Add(tmpH);
                                }
                            }
                            if (_tmpForExampleQuest.questIcon != null)
                            {
                                newForExampleQuestScript.questIcon = _tmpForExampleQuest.questIcon;
                            }
                            if (_tmpForExampleQuest.orikList.Count > 0)
                            {
                                foreach (Criteria criteria in _tmpForExampleQuest.orikList)
                                {
                                    Criteria tmpC = newForExampleQuestScript.gameObject.AddComponent<Criteria>();
                                    CopyAll(criteria, tmpC);
                                    newForExampleQuestScript.orikList.Add(tmpC);
                                }
                            }
                            if (_tmpForExampleQuest.rewards.Count > 0)
                            {
                                foreach (Reward reward in _tmpForExampleQuest.rewards)
                                {
                                    Reward tmpR = newForExampleQuestScript.gameObject.AddComponent<Reward>();
                                    CopyAll(reward, tmpR);
                                    newForExampleQuestScript.rewards.Add(tmpR);
                                }
                            }
                            if (_tmpForExampleQuest.optionalCriterias.Count > 0)
                            {
                                foreach (Criteria criteria in _tmpForExampleQuest.optionalCriterias)
                                {
                                    Criteria tmpC = newForExampleQuestScript.gameObject.AddComponent<Criteria>();
                                    CopyAll(criteria, tmpC);
                                    newForExampleQuestScript.optionalCriterias.Add(tmpC);
                                }
                            }
                            if (_tmpForExampleQuest.optionalRewards.Count > 0)
                            {
                                foreach (Reward reward in _tmpForExampleQuest.optionalRewards)
                                {
                                    Reward tmpR = newForExampleQuestScript.gameObject.AddComponent<Reward>();
                                    CopyAll(reward, tmpR);
                                    newForExampleQuestScript.optionalRewards.Add(tmpR);
                                }
                            }
                            tmpGameObject.name = "";
                            selectedForExampleQuest = newQuest.GetComponent<ForExampleQuest>();
                            DestroyImmediate(_tmpForExampleQuest.gameObject);
                        ...
                        case TypeToAttach.Criteria: //For attaching a criteria
                            ...
                            tp = GetComponentTypeByName(tmpGameObject.name.Replace(" ", ""));
                            tmpGameObject.AddComponent(tp);
                            GameObject criteriaPrefab = (GameObject)AssetDatabase.LoadAssetAtPath("Assets/CustomQuest/Assets/Prefabs/Criterias/" + tmpGameObject.name + ".prefab", typeof(GameObject));
                            criteriaPrefab.AddComponent(tp);
                            if (AssetDatabase.LoadAssetAtPath("Assets/CustomQuest/Assets/Prefabs/Criterias/" + tmpGameObject.name + ".prefab", typeof(GameObject)) != null)
                            {
                                AssetDatabase.CreateFolder("Assets/CustomQuest/Assets/Prefabs/Criterias", tmpGameObject.name);
                                AssetDatabase.MoveAsset("Assets/" + tmpGameObject.name.Replace(" ", "") + ".cs", "Assets/CustomQuest/Assets/Prefabs/Criterias/" + tmpGameObject.name + "/" + tmpGameObject.name.Replace(" ", "") + ".cs");
                                AssetDatabase.MoveAsset("Assets/CustomQuest/Assets/Prefabs/Criterias/" + tmpGameObject.name + ".prefab", "Assets/CustomQuest/Assets/Prefabs/Criterias/" + tmpGameObject.name + "/" + tmpGameObject.name + ".prefab");
                            }
                            else
                            {
                                AssetDatabase.MoveAsset("Assets/" + tmpGameObject.name.Replace(" ", "") + ".cs", "Assets/CustomQuest/Assets/Prefabs/Criterias/" + tmpGameObject.name + "/" + tmpGameObject.name.Replace(" ", "") + ".cs");
                        ...
                        case TypeToAttach.Reward: // For attaching a reward
                            ...
                            GameObject rewardPrefab = (GameObject)AssetDatabase.LoadAssetAtPath("Assets/CustomQuest/Assets/Prefabs/Rewards/" + tmpGameObject.name + ".prefab", typeof(GameObject));
                            rewardPrefab.AddComponent(tp);
                            if (AssetDatabase.LoadAssetAtPath("Assets/CustomQuest/Assets/Prefabs/Rewards/" + tmpGameObject.name + ".prefab", typeof(GameObject)) != null)
                            {
                                AssetDatabase.CreateFolder("Assets/CustomQuest/Assets/Prefabs/Rewards", tmpGameObject.name);
                                AssetDatabase.MoveAsset("Assets/" + tmpGameObject.name.Replace(" ", "") + ".cs", "Assets/CustomQuest/Assets/Prefabs/Rewards/" + tmpGameObject.name + "/" + tmpGameObject.name.Replace(" ", "") + ".cs");
                                AssetDatabase.MoveAsset("Assets/CustomQuest/Assets/Prefabs/Rewards/" + tmpGameObject.name + ".prefab", "Assets/CustomQuest/Assets/Prefabs/Rewards/" + tmpGameObject.name + "/" + tmpGameObject.name + ".prefab");
                            }
                            else
                            {
                                AssetDatabase.MoveAsset("Assets/" + tmpGameObject.name.Replace(" ", "") + ".cs", "Assets/CustomQuest/Assets/Prefabs/Rewards/" + tmpGameObject.name + "/" + tmpGameObject.name.Replace(" ", "") + ".cs");
    ...
    private void OnGUIPrefabs()
    {
        ...
        switch (prefabToolBarInt)
        {
            case 0: //Quest prefabs
                ...
                if (allQuests.ElementAtOrDefault(0) != null && selectedForExampleQuest == null)
                {
                    selectedForExampleQuest = allQuests[0];
                }
                ...
                GUILayout.Space(5);
                if (selectedForExampleQuest != null)
                {
                    if (GUILayout.Button(new GUIContent("-", "Click here to delete " + selectedForExampleQuest.questName), GUILayout.Width(38), GUILayout.Height(32)))
                    {
                        QuestDeleteWindow window = QuestDeleteWindow.Instance; // Gets the window, or creates a new one if none exists
                        window.SetQuestEditor(this, selectedForExampleQuest); //Sets the editor in the QuestDeleteWindow
                ...
                if (selectedForExampleQuest != null)
                {
                    if (GUILayout.Button(new GUIContent("Λ", "Click here to move " + selectedForExampleQuest.questName + " one up."), GUILayout.Width(38), GUILayout.Height(32)))
                    {
                        int index1 = allQuests.IndexOf(selectedForExampleQuest);
                        if (index1 > 0)
                        {
                            ForExampleQuest tmpForExampleQuest = allQuests[index1 - 1];
                            allQuests[index1 - 1] = selectedForExampleQuest;
                            allQuests[index1] = tmpForExampleQuest;
                ...
                if (selectedForExampleQuest != null)
                {
                    if (GUILayout.Button(new GUIContent("V", "Click here to move " + selectedForExampleQuest.questName + " one down"), GUILayout.Width(38), GUILayout.Height(32)))
                    {
                        int index1 = allQuests.IndexOf(selectedForExampleQuest);
        ...
        switch (prefabToolBarInt)
        {
            case 0:
                ...
                if (selectedForExampleQuest)
                {
                    ...
                    GUILayout.Space(4);
                    GUI.skin = null;
                    selectedForExampleQuest.questIcon = (Sprite)EditorGUILayout.ObjectField(selectedForExampleQuest.questIcon, typeof(Sprite), true, GUILayout.Height(34), GUILayout.Width(34));
                    GUI.skin = thisGUISkin;
                    ...
                    selectedForExampleQuest.questName = EditorGUILayout.TextField(selectedForExampleQuest.questName, thisGUISkin.GetStyle("TitleName"), GUILayout.Height(thisGUISkin.GetStyle("TitleName").CalcHeight(new GUIContent(selectedForExampleQuest.questName), position.width - 440)), GUILayout.Width(position.width - 440)); //TODO: Change name of script/Folder too

                    EditorGUILayout.EndVertical();
                    if (selectedForExampleQuest.questName != tmpQuestName)
                    { //Starts the change of the name
                        tmpQuestName = selectedForExampleQuest.questName;
                        lastName = selectedForExampleQuest.name;
                        updateUI = true;
                        updateCounter = 1;
                    ...
                    if (selectedForExampleQuest.isCustomScript != true)
                    {
                    ...
                    {
                        AssetDatabase.OpenAsset(AssetDatabase.LoadAssetAtPath("Assets/CustomQuest/Assets/Prefabs/Quests/" + selectedForExampleQuest.name + "/" + selectedForExampleQuest.name + ".cs", typeof(UnityEngine.Object)));
                    }
                    ...
                    {
                        UnityEngine.Object foundAsset = AssetDatabase.LoadAssetAtPath(AssetDatabase.GetAssetPath(selectedForExampleQuest), typeof(UnityEngine.Object));
                    ...
                    selectedForExampleQuest.description = EditorGUILayout.TextArea(selectedForExampleQuest.description, GUILayout.ExpandHeight(true), GUILayout.Width(position.width - 385));
                    EditorGUILayout.EndHorizontal();
                    ...
                    selectedForExampleQuest.toolTip = EditorGUILayout.TextArea(selectedForExampleQuest.toolTip, EditorStyles.textArea, GUILayout.ExpandHeight(true), GUILayout.Width(position.width - 385));
                    EditorGUILayout.EndHorizontal();
                    ...
                    //Quest settings
                    selectedForExampleQuest.showSettings = EditorGUILayout.Foldout(selectedForExampleQuest.showSettings, new GUIContent("Quest Settings", "Settings for the quests"), true, boldStyle);
                    if (selectedForExampleQuest.showSettings)
                    {
                        EditorGUILayout.BeginVertical(EditorStyles.helpBox);
                        EditorGUILayout.BeginHorizontal();
                        ...
                        selectedForExampleQuest.startAvailability = EditorGUILayout.Toggle(selectedForExampleQuest.startAvailability, GUILayout.Width(15));
                        GUILayout.FlexibleSpace();
                        ...
                        selectedForExampleQuest.constantAvailability = EditorGUILayout.Toggle(selectedForExampleQuest.constantAvailability, GUILayout.Width(15));
                        EditorGUILayout.EndHorizontal();
                        ...
                        selectedForExampleQuest.autoComplete = EditorGUILayout.Toggle(selectedForExampleQuest.autoComplete, GUILayout.MaxWidth(15));
                        GUILayout.FlexibleSpace();
                        ...
                        selectedForExampleQuest.pickUpAble = EditorGUILayout.Toggle(selectedForExampleQuest.pickUpAble, GUILayout.Width(15));
                        EditorGUILayout.EndHorizontal();

                        EditorGUILayout.BeginHorizontal();
                        EditorGUILayout.LabelField(new GUIContent("Don't Delete", "If true, this quest will never be deleted."), GUILayout.Width(150));
                        selectedForExampleQuest.dontDelete = EditorGUILayout.Toggle(selectedForExampleQuest.dontDelete, GUILayout.Width(15));
                        GUILayout.FlexibleSpace();
                        ...
                        selectedForExampleQuest.singleComplete = EditorGUILayout.Toggle(selectedForExampleQuest.singleComplete, GUILayout.Width(15));
                        EditorGUILayout.EndHorizontal();
                        ...
                        selectedForExampleQuest.timed = EditorGUILayout.Toggle(selectedForExampleQuest.timed, GUILayout.Width(15));
                        if (selectedForExampleQuest.timed)
                        {
                            selectedForExampleQuest.time = EditorGUILayout.FloatField(selectedForExampleQuest.time);
                        ...
                        selectedForExampleQuest.repeatable = EditorGUILayout.Toggle(selectedForExampleQuest.repeatable, GUILayout.Width(15));
                        if (selectedForExampleQuest.repeatable)
                        {
                            selectedForExampleQuest.repeatableTime = EditorGUILayout.FloatField(selectedForExampleQuest.repeatableTime);
                            EditorGUI.LabelField(GUILayoutUtility.GetLastRect(), new GUIContent("", "The time in seconds the player have to complete this quest"));
                        }
                        EditorGUILayout.EndHorizontal();
                        ...
                        selectedForExampleQuest.startSpawningOnDiscover = EditorGUILayout.Toggle(selectedForExampleQuest.startSpawningOnDiscover, GUILayout.Width(15));
                        GUILayout.FlexibleSpace();
                        ...
                        selectedForExampleQuest.noSpawnIfNoPlayer = EditorGUILayout.Toggle(selectedForExampleQuest.noSpawnIfNoPlayer, GUILayout.Width(15));
                        EditorGUILayout.EndHorizontal();
                        ...
                        {
                            foreach (Criteria c in selectedForExampleQuest.orikList)
                            {
                        ...
                        selectedForExampleQuest.matchOptionalLevels = EditorGUILayout.Toggle(selectedForExampleQuest.matchOptionalLevels, GUILayout.Width(15));
                        EditorGUILayout.EndHorizontal();
                    ...
                    if (GUILayout.Button(new GUIContent("Add Criterion", "Click to add a criterion to this quest"), GUILayout.Height(22), GUILayout.MinWidth(150), GUILayout.MaxWidth(150)))
                    {
                        ...
                        menu.AddItem(new GUIContent("Empty"), false, CreateNewCriteriaCallBack, selectedForExampleQuest);
                    ...
                    if (R_CriteriaList == null) { SelectQuest(selectedForExampleQuest); }
                    ...
                    selectedForExampleQuest.showThresholds = EditorGUILayout.Foldout(selectedForExampleQuest.showThresholds, new GUIContent("Tresholds", "Click here to show the thresholds for the different levels of criteria"), true);
                    EditorGUI.indentLevel -= 1;
                    if (selectedForExampleQuest.showThresholds)
                    {
                        EditorGUILayout.BeginVertical(EditorStyles.helpBox); //Criterias
                        ...
                        if (Event.current.type == EventType.Layout)
                        {
                            foreach (Criteria c in selectedForExampleQuest.orikList)
                            {
                                if (selectedForExampleQuest.thresholds.Count <= c.Level)
                                {
                                    selectedForExampleQuest.thresholds.Add(0);
                                }
                                if (c.Level > maxLevel)
                                {
                                    maxLevel = c.Level;
                            ...
                            maxLevel += 1;
                            if (maxLevel < selectedForExampleQuest.thresholds.Count)
                            {
                                selectedForExampleQuest.thresholds.RemoveRange(maxLevel, selectedForExampleQuest.thresholds.Count - maxLevel);
                            }
                        }

                        for (int i = 0; i < selectedForExampleQuest.thresholds.Count; i++)
                        {
                            EditorGUILayout.BeginHorizontal();
                            EditorGUILayout.LabelField(i.ToString(), GUILayout.Width(30));
                            selectedForExampleQuest.thresholds[i] = EditorGUILayout.IntField(selectedForExampleQuest.thresholds[i]);
                            EditorGUILayout.EndHorizontal();
                        }
                        EditorGUILayout.EndVertical();
                    }
                    ...
                    {
                        GenericMenu menu = new GenericMenu();
                        menu.AddItem(new GUIContent("Empty"), false, CreateNewRewardCallBack, selectedForExampleQuest);
                    ...
                    EditorGUILayout.EndHorizontal();
                    GUI.backgroundColor = Color.white;
                    if (R_RewardList == null) { SelectQuest(selectedForExampleQuest); }
                    ...
                    if (!CustomQuestSettings.SettingsHolder.optional)
                    {
                        ...
                        {
                            GenericMenu menu = new GenericMenu();
                            menu.AddItem(new GUIContent("Empty"), false, CreateNewOptionalCriteriaCallBack, selectedForExampleQuest);
                            foreach (Criteria c in allCriterias)
                            {
                        ...
                        EditorGUILayout.EndHorizontal();
                        GUI.backgroundColor = Color.white;
                        if (R_OptionalCriteriaList == null) { SelectQuest(selectedForExampleQuest); }
                        ...
                        EditorGUI.indentLevel += 1;
                        selectedForExampleQuest.showOptionalThresholds = EditorGUILayout.Foldout(selectedForExampleQuest.showOptionalThresholds, new GUIContent("Optional Criteria Tresholds", "Click here to show the thresholds for the different levels of criterias"), true);
                        EditorGUI.indentLevel -= 1;
                        if (selectedForExampleQuest.showOptionalThresholds)
                        {
                            EditorGUILayout.BeginVertical(EditorStyles.helpBox); //Criterias
                            ...
                            if (Event.current.type == EventType.Layout)
                            {
                                foreach (Criteria c in selectedForExampleQuest.optionalCriterias)
                                {
                                    if (selectedForExampleQuest.optionalThresholds.Count <= c.Level)
                                    {
                                        selectedForExampleQuest.optionalThresholds.Add(0);
                                    }
                                    if (c.Level > maxLevel)
                                    {
                                        maxLevel = c.Level;
                                ...
                                maxLevel += 1;
                                if (maxLevel < selectedForExampleQuest.optionalThresholds.Count)
                                {
                                    selectedForExampleQuest.optionalThresholds.RemoveRange(maxLevel, selectedForExampleQuest.optionalThresholds.Count - maxLevel);
                                }
                            }

                            for (int i = 0; i < selectedForExampleQuest.optionalThresholds.Count; i++)
                            {
                                EditorGUILayout.BeginHorizontal();
                                EditorGUILayout.LabelField(i.ToString(), GUILayout.Width(30));
                                selectedForExampleQuest.optionalThresholds[i] = EditorGUILayout.IntField(selectedForExampleQuest.optionalThresholds[i]);
                                EditorGUILayout.EndHorizontal();
                            }
                            EditorGUILayout.EndVertical();
                        }
                    ...
                    if (!CustomQuestSettings.HideOptional)
                    {
                        ...
                        selectedForExampleQuest.completedOptionalThreshold = EditorGUILayout.IntField(/*new GUIContent("Threshold", "The threshold for getting the optional rewards."),*/ selectedForExampleQuest.completedOptionalThreshold, GUILayout.Height(22), GUILayout.Width(20), GUILayout.ExpandWidth(false));
                        ...
                        {
                            GenericMenu menu = new GenericMenu();
                            menu.AddItem(new GUIContent("Empty"), false, CreateNewOptionalRewardCallBack, selectedForExampleQuest);
                            foreach (Reward r in allRewards)
                            {
                        ...
                        EditorGUILayout.EndHorizontal();
                        GUI.backgroundColor = Color.white;
                        if (R_OptionalRewardsList == null) { SelectQuest(selectedForExampleQuest); }
            ...
            case 1:
                ...
                if (selectedCriteria != null)
                {
                    ...
                    {
                        AssetDatabase.OpenAsset(AssetDatabase.LoadAssetAtPath("Assets/CustomQuest/Assets/Prefabs/Criterias/" + selectedCriteria.name + "/" + selectedCriteria.name + ".cs", typeof(UnityEngine.Object)));
                    }
                    // Select prefab button
            ...
            case 2:
                ...
                if (selectedReward != null)
                {
                    ...
                    {
                        AssetDatabase.OpenAsset(AssetDatabase.LoadAssetAtPath("Assets/CustomQuest/Assets/Prefabs/Rewards/" + selectedReward.name + "/" + selectedReward.name + ".cs", typeof(UnityEngine.Object)));
                    }
                    // Select prefab button
                    if (GUI.Button(new Rect(rect.x + rect.width - 90, rect.y, 60, EditorGUIUtility.singleLineHeight), new GUIContent("Select", "Click here to select the prefab")))
                    {
    ...
    private void OnGUIQuestsInScene()
    {
        ...
        nodes.Clear();
        foreach (ForExampleQuest q in FindObjectsOfType<ForExampleQuest>())
        {
            if (q.questNode != null)
            {
        ...
        foreach (QuestNode qn in nodes)
        {
            if (qn != null) //TODO: make a check to tell the player if the quest this questnode is displaying, for some reason is null
            {
                if (qn.forExampleQuest != null)
                {
                    ...
                    qn.Rectangle = qn.forExampleQuest.rectangleNode;
                    GUI.backgroundColor = new Color32(132, 132, 132, 255);
                    GUI.skin = null;
                    qn.Rectangle = ClampToScreen(GUI.Window(cnt, qn.Rectangle, DoQuestNodeWindow, qn.forExampleQuest.questName), Screen.width - 150, Screen.height - 50);
                    GUI.skin = thisGUISkin;
                    GUI.backgroundColor = Color.white;
                    qn.windowID = cnt;
                    cnt++;
                ...
                {
                    object o = EditorUtility.InstanceIDToObject(qn.forExampleQuest.GetInstanceID()); //Tries to find the quest again
                    qn.forExampleQuest = (ForExampleQuest)o;
                    if (qn.forExampleQuest == null) //Remove if it found none
                    {
                        nodes.Remove(qn);
                    }
                    break;
                ...
                for (int i = 0; i < nodeToDelete.allEdges.Count; i++)
                {
                    ...
                    { //Left
                        for (int j = 0; j < qe.connections.Count; j++)
                        {
                            ...
                            qc.leftEdge.questNode.forExampleQuest.questsToUnlock.Remove(qc.rightEdge.questNode.forExampleQuest);
                            qc.leftEdge.connections.Remove(qc);
                        }
                    }
                    else
                    { //Right
                        ...
                        {
                            QuestConnection qc = qe.connections[j];
                            qc.rightEdge.questNode.forExampleQuest.unCompletedQuests.Remove(qc.leftEdge.questNode.forExampleQuest);
                            qc.rightEdge.connections.Remove(qc);
                        }
                    }
                }
                DestroyImmediate(nodeToDelete.forExampleQuest.gameObject);
    ...
    private void CreateQuest(GameObject go)
    {
        ...
        ForExampleQuest forExampleQuest = tmpGo.GetComponent<ForExampleQuest>();
        forExampleQuest.prefab = go;
        QuestNode qn = CreateInstance<QuestNode>();
        qn.forExampleQuest = forExampleQuest;
        forExampleQuest.questNode = qn;
        qn.Start();
        if (!EditorSceneManager.GetActiveScene().isDirty)
        {
            if (!Application.isPlaying)
    ...
    private void DoQuestNodeWindow(int id)
    {
        if (nodes.ElementAtOrDefault(id) != null)
        {
            ...
            {
                object o = EditorUtility.InstanceIDToObject(qn.forExampleQuest.GetInstanceID());
                Selection.activeObject = (ForExampleQuest)o;
            }
            if (qn.startEdge)
            {
                ...
                { // Mouse Click
                    ...
                    { // Right Click
                        ...
                        if (currentEvent.type == EventType.MouseDown)
                        {
                            ...
                            {
                                menu.AddItem(new GUIContent("Delete connection to: " + c.leftEdge.questNode.forExampleQuest.questName), false, ConnectionDeleteCallBack, c); //TODO: somehow make this delete the connections
                ...
                if (currentEvent.isMouse && qn.completeEdge.rect.Contains(currentEvent.mousePosition))
                {
                    ...
                    { // Right Click
                        ...
                        if (currentEvent.type == EventType.MouseDown)
                        {
                            ...
                            {
                                menu.AddItem(new GUIContent("Delete connection to: " + c.rightEdge.questNode.forExampleQuest.questName), false, ConnectionDeleteCallBack, c);
                                if (connectionDeleted)
                                {
                                    connectionDeleted = false;
                                    break;
    ...
    private void MakeQuestConnection(QuestEdge from, QuestEdge to)
    {
        if (from.left != to.left)
        {
            ...
            if (!skip)
            {
                ...
                from.questNode.forExampleQuest.questsToUnlock.Add(to.questNode.forExampleQuest);
                to.questNode.forExampleQuest.unCompletedQuests.Add(from.questNode.forExampleQuest);
                if (!EditorSceneManager.GetActiveScene().isDirty)
                {
                    if (!Application.isPlaying)
                    {
    ...
    private void RenamePrefab(GameObject go)
    {
        ...
        {
            case TypeToRename.Quest:
                ForExampleQuest q = go.GetComponent<ForExampleQuest>();
                q.name = q.questName;
                AssetDatabase.RenameAsset(AssetDatabase.GetAssetPath(q), q.questName);
                AssetDatabase.RenameAsset("Assets/CustomQuest/Assets/Prefabs/Quests/" + lastName + "/" + lastName + ".cs", q.questName); //Renames the quest script //TODO: make it also rename the name of the script
                AssetDatabase.RenameAsset("Assets/CustomQuest/Assets/Prefabs/Quests/" + lastName, q.questName); //TODO: Check for invalid names (Unity kinda does that for you?)
                break;
            ...
            case TypeToRename.Reward:
                ...
                AssetDatabase.RenameAsset("Assets/CustomQuest/Assets/Prefabs/Rewards/" + lastName, r.rewardName);
                break;

            case TypeToRename.Criteria:
                Criteria c = go.GetComponent<Criteria>();
                c.name = c.criteriaName;
                //if (AssetDatabase.IsValidFolder("Assets/CustomQuest/Assets/Prefabs/Criterias/" + lastName))
                //{
                //    System.IO.File.Move("Assets/CustomQuest/Assets/Prefabs/Criterias/" + lastName + "/" + lastName + ".cs", "Assets/CustomQuest/Assets/Prefabs/Criterias/" + lastName + "/" + c.criteriaName + ".cs");
                //}
                AssetDatabase.RenameAsset(AssetDatabase.GetAssetPath(c), c.criteriaName);
                //AssetDatabase.RenameAsset("Assets/CustomQuest/Assets/Prefabs/Criterias/" + lastName + "/" + lastName + ".cs", c.criteriaName); //Renames the quest script //TODO: make it also rename the name of the script)
                AssetDatabase.RenameAsset("Assets/CustomQuest/Assets/Prefabs/Criterias/" + lastName, c.criteriaName);
                break;
        }
    }
    ...
    {
        string[] guids = AssetDatabase.FindAssets("t:GameObject", new string[] { "Assets/CustomQuest/Assets/Prefabs/Quests" });
        foreach (string guid in guids)
        {
            ...
            if (newQuest != null)
            {
                if (newQuest.GetComponent<ForExampleQuest>())
                {
                    if (!allQuests.Contains(newQuest.GetComponent<ForExampleQuest>()))
                    {
                        allQuests.Add(newQuest.GetComponent<ForExampleQuest>());
                    }
                }
            }
        }
    ...
    private void CreateNewQuest()
    {
        ...
        gameObject.AddComponent<ForExampleQuest>();
        PrefabUtility.CreatePrefab("Assets/CustomQuest/Assets/Prefabs/Quests/tmpQuestName.prefab", gameObject);
        GameObject tmpNewGameObject = (GameObject)AssetDatabase.LoadAssetAtPath("Assets/CustomQuest/Assets/Prefabs/Quests/tmpQuestName.prefab", typeof(GameObject));
        tmpNewGameObject.name = "tmpQuestName";
        tmpNewGameObject.GetComponent<ForExampleQuest>().questName = tmpNewGameObject.name;
        SelectQuest(tmpNewGameObject.GetComponent<ForExampleQuest>());
        DestroyImmediate(gameObject);
        ClearFocus();
        UpdateQuestList();
    }

    public void ConvertToCustomQuest(ForExampleQuest q)
    {
        ...
        tmpGameObject.AddComponent<ForExampleQuest>();

        _tmpForExampleQuest = Instantiate(q);

        if (!AssetDatabase.IsValidFolder("Assets/CustomQuest/Assets/Prefabs/Quests/" + tmpGameObject.name))
        {
            AssetDatabase.CreateFolder("Assets/CustomQuest/Assets/Prefabs/Quests", tmpGameObject.name);
        }
        TextAsset templateTextFile = (TextAsset)AssetDatabase.LoadAssetAtPath("Assets/CustomQuest/Assets/Templates/QuestTemplate.txt", typeof(TextAsset));
    ...
    public void DeleteQuest(ForExampleQuest q)
    {
        allQuests.Remove(q);
        if (AssetDatabase.LoadAssetAtPath("Assets/CustomQuest/Assets/Prefabs/Quests/" + q.name + ".prefab", typeof(GameObject)))
        {
            AssetDatabase.DeleteAsset("Assets/CustomQuest/Assets/Prefabs/Quests/" + q.name + ".prefab");
        }
        else
        {
            AssetDatabase.DeleteAsset("Assets/CustomQuest/Assets/Prefabs/Quests/" + q.name);
        }
        ClearFocus();
    }

    private void SelectQuest(ForExampleQuest q)
    {
        if (updateUI == true)
        {
            RenamePrefab(selectedForExampleQuest.gameObject);
        ...
        ClearFocus();
        selectedForExampleQuest = q;
        tmpQuestName = selectedForExampleQuest.questName;
        R_CriteriaList = CreateCriteriaList(selectedForExampleQuest.orikList);
        R_RewardList = CreateRewardList(selectedForExampleQuest.rewards);
        R_OptionalCriteriaList = CreateCriteriaList(selectedForExampleQuest.optionalCriterias);
        R_OptionalRewardsList = CreateRewardList(selectedForExampleQuest.optionalRewards);
    }
    ...
    {
        string[] guids = AssetDatabase.FindAssets("t:GameObject", new string[] { "Assets/CustomQuest/Assets/Prefabs/Criterias" });
        foreach (string guid in guids)
        {
            string assetPath = AssetDatabase.GUIDToAssetPath(guid);
    ...
    private void CreateNewCriteriaPrefab()
    {
        ...
        PrefabUtility.CreatePrefab("Assets/CustomQuest/Assets/Prefabs/Criterias/tmpCriteriaName.prefab", gameObject);
        GameObject tmpNewGameObject = (GameObject)AssetDatabase.LoadAssetAtPath("Assets/CustomQuest/Assets/Prefabs/Criterias/tmpCriteriaName.prefab", typeof(GameObject));
    ...
    {
        allCriterias.Remove(c);
        if (AssetDatabase.LoadAssetAtPath("Assets/CustomQuest/Assets/Prefabs/Criterias/" + c.name + ".prefab", typeof(GameObject)))
        {
            AssetDatabase.DeleteAsset("Assets/CustomQuest/Assets/Prefabs/Criterias/" + c.name + ".prefab");
        }
        else
        {
            AssetDatabase.DeleteAsset("Assets/CustomQuest/Assets/Prefabs/Criterias/" + c.name);
        }
        ClearFocus();
    }
    ...
    private void AddNewCriteriaCallBack(object obj)
    {
        ...
        Criteria tmpCriteria = selectedForExampleQuest.gameObject.AddComponent<Criteria>();
        selectedForExampleQuest.orikList.Add(tmpCriteria);
    ...
    private void AddNewOptionalCriteriaCallBack(object obj)
    {
        Criteria c = (Criteria)obj;
        Criteria tmpCriteria = selectedForExampleQuest.gameObject.AddComponent<Criteria>();
        selectedForExampleQuest.optionalCriterias.Add(tmpCriteria);
        CopyAll(c, tmpCriteria);
    ...
    public void ConvertToCustomCriteria(Criteria c)
    {
        ...
        TextAsset templateTextFile = (TextAsset)AssetDatabase.LoadAssetAtPath("Assets/CustomQuest/Assets/Templates/CriteriaTemplate.txt", typeof(TextAsset));
        string contents = "";
    ...
    {
        selectedForExampleQuest.orikList.Remove(c);
        if (AssetDatabase.LoadAssetAtPath("Assets/CustomQuest/Assets/Prefabs/Quests/" + selectedForExampleQuest.name + "/" + c.criteriaName + ".cs", typeof(MonoScript)))
        {
            AssetDatabase.DeleteAsset("Assets/CustomQuest/Assets/Prefabs/Quests/" + selectedForExampleQuest.name + "/" + c.criteriaName + ".cs");
        }
        DestroyImmediate(c, true);
        ClearFocus();
    }
    ...
    {
        string[] guids = AssetDatabase.FindAssets("t:GameObject", new string[] { "Assets/CustomQuest/Assets/Prefabs/Rewards" });
        foreach (string guid in guids)
        {
            string assetPath = AssetDatabase.GUIDToAssetPath(guid);
    ...
    private void CreateNewRewardPrefab()
    {
        ...
        PrefabUtility.CreatePrefab("Assets/CustomQuest/Assets/Prefabs/Rewards/tmpRewardName.prefab", gameObject);
        GameObject tmpNewGameObject = (GameObject)AssetDatabase.LoadAssetAtPath("Assets/CustomQuest/Assets/Prefabs/Rewards/tmpRewardName.prefab", typeof(GameObject));
    ...
    {
        allRewards.Remove(r);
        if (AssetDatabase.LoadAssetAtPath("Assets/CustomQuest/Assets/Prefabs/Rewards/" + r.name + ".prefab", typeof(GameObject)))
        {
            AssetDatabase.DeleteAsset("Assets/CustomQuest/Assets/Prefabs/Rewards/" + r.name + ".prefab");
        }
        else
        {
            AssetDatabase.DeleteAsset("Assets/CustomQuest/Assets/Prefabs/Rewards/" + r.name);
        }
        ClearFocus();
    }
    ...
    {
        Reward r = (Reward)obj;
        Reward tmpReward = selectedForExampleQuest.gameObject.AddComponent<Reward>();
        selectedForExampleQuest.rewards.Add(tmpReward);
        CopyAll(r, tmpReward);
    }
    ...
    private void AddNewOptionalRewardCallBack(object obj)
    {
        Reward r = (Reward)obj;
        Reward tmpReward = selectedForExampleQuest.gameObject.AddComponent<Reward>();
        selectedForExampleQuest.optionalRewards.Add(tmpReward);
        CopyAll(r, tmpReward);
    ...
    public void ConvertToCustomReward(Reward r, ForExampleQuest q)
    {
        //Instantiate a new gameObject
        tmpGameObject = new GameObject();
        ...
        TextAsset templateTextFile = (TextAsset)AssetDatabase.LoadAssetAtPath("Assets/CustomQuest/Assets/Templates/RewardTemplate.txt", typeof(TextAsset));
        string contents = "";
    ...
    {
        selectedForExampleQuest.rewards.Remove(r);
        if (AssetDatabase.LoadAssetAtPath("Assets/CustomQuest/Assets/Prefabs/Quests/" + selectedForExampleQuest.name + "/" + r.rewardName + ".cs", typeof(MonoScript)))
        {
            AssetDatabase.DeleteAsset("Assets/CustomQuest/Assets/Prefabs/Quests/" + selectedForExampleQuest.name + "/" + r.rewardName + ".cs");
        }
        DestroyImmediate(r, true);
        ClearFocus();
    }
