...
public class Criteria : MonoBehaviour
{
    ...
    private ForExampleQuest forExampleQuest;
    ...
    /// A reference to the Quest script.
    /// </summary>
    public ForExampleQuest ForExampleQuest { get { if (forExampleQuest == null) { forExampleQuest = GetComponent<ForExampleQuest>(); } return forExampleQuest; } set { forExampleQuest = value; } }
    ...
    public virtual void Start()
    {
        ...
        ForExampleQuest = GetComponent<ForExampleQuest>();
    }
    ...
    public virtual void StartCriteria(CQPlayerObject player)
    {
        ...
        if (forExampleQuest.startSpawningOnDiscover)
        {
            foreach (SpawnZone zone in spawnZones)
            {
                ...
                {
                    zone.Spawn = true;
                    forExampleQuest.SpawnQuestObjects(criteriaObject, zone.initialSpawnAmount, zone);
                }
            }
        }
    }
    ...
    public virtual void Progress(CQPlayerObject player, CQExamplePlayer unit)
    {
        if (playerProgression.ContainsKey(player))
        {
            ...
            if (playerProgression[player] >= amount)
            {
                ForExampleQuest.CriteriaCompleted(unit, player, this);
            }
        }
    }
    ...
    public virtual void Complete(CQPlayerObject player, CQExamplePlayer unit)
    {
        ...
        {
            bool despawn = true;
            foreach (CQPlayerObject p in forExampleQuest.playersUnCompleted)
            {
                if (forExampleQuest.activeCriterias[p].Contains(this))
                {
                    despawn = false;
                }
            }
    ...
    public virtual void Fail(CQPlayerObject player)
    {
        ...
        forExampleQuest.CriteriaFailed(player, this);
