...
public class QuestUI : MonoBehaviour
{
    ...
    /// </summary>
    public List<ForExampleQuest> canvasQuests = new List<ForExampleQuest>();
    ...
    /// </summary>
    public Dictionary<CQPlayerObject, List<ForExampleQuest>> activeQuestPopUpQuests = new Dictionary<CQPlayerObject, List<ForExampleQuest>>();
    ...
    private void CanvasUpdate()
    {
        ...
        {
            if (qUI.forExampleQuest != null) //Checks if the quest of the questUI is null
            { //Updates the text of the questUI to match the quest
                if (questHandler.availableQuests[questHandler.SelectedPlayer].Contains(qUI.forExampleQuest))
                {
                    qUI.questName.text = qUI.forExampleQuest.questName;
                    qUI.description.text = qUI.forExampleQuest.description;
                    qUI.questIcon.sprite = qUI.forExampleQuest.questIcon; //Updates the icon to match the quest
                ...
                {
                    ResetQuest(qUI.forExampleQuest);
                    break;
                }
            }
            else
        ...
        foreach (ForExampleQuest q in questHandler.availableQuests[questHandler.SelectedPlayer])
        {// Goes through every available quest, for the selectedPlayer
            if (!canvasQuests.Contains(q)) // Checks if canvasQuests constains the current quest
            {
                ...
                if (questTemplate != null)
                {
                    ...
                    qUI.forExampleQuest = q; //Adds the quest to the script
                    ...
                    qUI.questName.text = qUI.forExampleQuest.questName; //Updates the ui text name to match the script
                    qUI.description.text = qUI.forExampleQuest.description; //Updates the description to match the script
                    qUI.questIcon.sprite = qUI.forExampleQuest.questIcon; //Updates the icon to match the quest
                    ...
                    List<Criteria> allCriterias = new List<Criteria>(qUI.forExampleQuest.activeCriterias[questHandler.SelectedPlayer]);
                    allCriterias.AddRange(qUI.forExampleQuest.activeOptionalCriterias[questHandler.SelectedPlayer]);
                    foreach (Criteria c in qUI.forExampleQuest.activeCriterias[questHandler.SelectedPlayer]) //Goes through every criteria in this quest
                    {
                    ...
                    foreach (Reward r in qUI.forExampleQuest.rewards) //Goes through every reward in this quest //TODO: Correct comments
                    {
                    ...
                    if (qUI.forExampleQuest.activeOptionalCriterias[questHandler.SelectedPlayer].Count > 0 || qUI.forExampleQuest.optionalRewards.Count > 0)
                    {
                    ...
                    foreach (Criteria c in qUI.forExampleQuest.activeOptionalCriterias[questHandler.SelectedPlayer]) //Goes through every criteria in this quest
                    {
                        if (!qUI.criterias.Contains(c)) //Checks if the quest UI scripts criteria list contains this criteria
                        {
                            qUI.criterias.Add(c); //Adds the criteria the list, so it does not get in here again
                    ...
                    foreach (Reward r in qUI.forExampleQuest.optionalRewards) //Goes through every reward in this quest //TODO: Correct comments
                    {
                        if (!qUI.rewards.Contains(r)) //Checks if the quest UI scripts reward list contains this reward
                        {
                            qUI.rewards.Add(r); //Adds the reward the list, so it does not get in here again
    ...
    public void ResetQuest(ForExampleQuest q)
    {
        for (int i = 0; i < questUis.Count; i++)
        {
            ...
            canvasQuests.Remove(qUI.forExampleQuest);
    ...
    public void UpdateQuestTracker()
    {
        ...
        foreach (ForExampleQuest q in QuestHandler.Instance.availableQuests[QuestHandler.Instance.SelectedPlayer])
        {
    ...
    public void StartQuestPopUp(QuestGiver questGiver, CQPlayerObject player, ForExampleQuest forExampleQuest)
    {
        if (!activeQuestPopUpQuests.ContainsKey(player))
        {
            activeQuestPopUpQuests.Add(player, new List<ForExampleQuest>());
        }
        if (!activeQuestPopUpQuests[player].Contains(forExampleQuest))
        {
            if (!forExampleQuest.playersUnCompleted.Contains(player))
            {
                if (!forExampleQuest.playersCompleted.Contains(player) || (forExampleQuest.repeatable && forExampleQuest.remainingRepeatableTime[player] <= 0))
                {
                    ...
                    questPopUp.forExampleQuest = forExampleQuest;
                    activeQuestPopUpQuests[player].Add(forExampleQuest);
    ...
    public void UpdateSelectionCircle(int highlightedItem)
    {
        ...
        try
        {
            ForExampleQuest highlightedForExampleQuest = QuestHandler.Instance.availableQuests[QuestHandler.Instance.selectedPlayer][highlightedItem - 1];
            middleText.text = highlightedForExampleQuest.questName;
            questNameText.text = highlightedForExampleQuest.questName;
            descriptionText.text = highlightedForExampleQuest.description;
            contentHolder = "Criteria";
            contentHolder += "\n";
            foreach (Criteria c in highlightedForExampleQuest.orikList)
            {
                contentHolder += c.criteriaName + "  " + c.playerProgression[QuestHandler.Instance.selectedPlayer] + " / " + c.amount + "\n";
            }
            if (highlightedForExampleQuest.unCompletedCriterias[QuestHandler.Instance.selectedPlayer].Count <= 0)
            {
                foreach (HandInObject h in highlightedForExampleQuest.handInObjects)
                {
            ...
            foreach (Criteria c in highlightedForExampleQuest.unCompletedCriterias[QuestHandler.Instance.selectedPlayer])
            {
                foreach (SpawnZone zone in c.spawnZones)
                {
                    foreach (Image img in zone.spawnAreaObject.GetComponentsInChildren<Image>())
            ...
            foreach (Reward r in highlightedForExampleQuest.rewards)
            {
    ...
    private void MouseClick(List<GameObject> RayHits)
    {
        ...
        if (qG != null)
        {
            ...
            if (Vector3.Distance(QuestHandler.Instance.SelectedPlayer.transform.position, qG.transform.position) <= qG.radius || qG.radius == 0) //TODO: From unit distance?
            {
                ...
                {
                    foreach (ForExampleQuest quest in qG.quests)
                    {
    ...
    {
        if (info.ForExampleQuest)
        {
            AddOnSreenMsg(5f, info.ForExampleQuest.questName + " Completed!", 20, Color.white); //Adds an on screen telling the player, he has completed this quest
        }
    }
    ...
    private void ResetQuestEvent(EventInfoHolder info)
    {
        if (info.ForExampleQuest)
        {
            ResetQuest(info.ForExampleQuest);
        }
    }
    ...
    {
        StartQuestPopUp(info.questGiver, info.player, info.ForExampleQuest);
    }
}